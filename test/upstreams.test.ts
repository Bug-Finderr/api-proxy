...
import { upstreamBase, rewriteToUpstream } from "../src/upstreams";
...
describe("upstreamBase", () => {
	it("defaults to the real hosts when no override is set", () => {
		...
		expect(upstreamBase("gemini", bare)).toBe("https://generativelanguage.googleapis.com");
	});
	it("maps gemini-openai to the gemini upstream", () => {
		expect(upstreamBase("gemini-openai", bare)).toBe("https://generativelanguage.googleapis.com");
	});
...
describe("rewriteToUpstream", () => {
	...
	it("targets a localhost override with port (the test seam)", () => {
		...
		const url = new URL("https://proxy.example/v1beta/models/x:streamGenerateContent?alt=sse");
