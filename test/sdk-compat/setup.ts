...
function providerFromPath(path: string): "openai" | "anthropic" | "gemini" {
	...
	if (path.includes(":generateContent") || path.includes(":streamGenerateContent") || path.startsWith("/v1beta/"))
		return "gemini";
...
function writeSse(res: http.ServerResponse, provider: string) {
	...
	if (provider === "anthropic") {
		...
		res.write('data: {"candidates":[{"content":{"parts":[{"text":"hi"}],"role":"model"},"finishReason":"STOP"}]}\n\n');
	} else {
		res.write('data: {"id":"x","object":"chat.completion.chunk","created":0,"model":"x","choices":[{"index":0,"delta":{"role":"assistant","content":"hi"},"finish_reason":null}]}\n\n');
...
export async function startMockUpstream(): Promise<MockUpstream> {
	...
	const server = http.createServer((req, res) => {
		...
		req.on("end", () => {
			...
			const stream =
				path.includes("streamGenerateContent") || /[?&]alt=sse/.test(path) || /"stream"\s*:\s*true/.test(body);
...
export async function startWorker(mockUrl: string): Promise<{ worker: Unstable_DevWorker; url: string }> {
