...
- **Live-verified against OpenAI (2026-07-05), not Gemini.** Through the deployed worker: Realtime returned `session.created` via both the Bearer slot and the smuggled-subprotocol slot (upstream negotiated `realtime`), and `/v1/responses` WS mode accepted the upgrade. Gemini Live remains mock-only (no key). The automated tests still run against a mock `ws` upstream by design.
