...
export const dashboardPage = () => html`<!doctype html>
	<html lang="en">
		...
		<body
			...
			hx-on:input="const t = event.target; if (t.type === 'datetime-local' && t.value.slice(11) === new Date().toTimeString().slice(0, 5)) { t.value = t.value.slice(0, 11) + '23:59'; if (t.closest('#rows')) htmx.trigger(t, 'commit'); else { t.blur(); try { t.showPicker() } catch {} } }"
