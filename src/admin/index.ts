...
async function isAuthed(req: Request, secret: string): Promise<boolean> {
	...
	const t = Number(ts);
	if (!ts || !sig || !Number.isFinite(t) || Date.now() / 1000 - t > MAX_AGE) return false;
	const sigBytes = fromHex(sig);
	if (!sigBytes) return false;
	// crypto.subtle.verify is constant-time; never compare HMACs with ===.
	return crypto.subtle.verify("HMAC", await hmacKey(secret), sigBytes, new TextEncoder().encode(ts));
}
...
app.get("/logout", (c) =>
	c.body(null, 302, { Location: "/admin", "Set-Cookie": `${COOKIE}=; Path=/admin; HttpOnly; Secure; SameSite=Strict; Max-Age=0` }),
);
...
app.put("/api/tokens/:hash", async (c) => {
	...
	return c.html(tokenRow({ hash, ...meta }), 200, { "HX-Trigger": "tokens-changed" });
});
